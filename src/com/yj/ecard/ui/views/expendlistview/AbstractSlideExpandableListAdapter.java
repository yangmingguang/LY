...
public abstract class AbstractSlideExpandableListAdapter extends WrapperListAdapterImpl {
	...
	*/
	...
	private void notifiyExpandCollapseListener(int type, View view, View parentView, int position) {
		if (expandCollapseListener != null) {
			if (type == ExpandCollapseAnimation.EXPAND) {
				expandCollapseListener.onExpand(view, parentView, position);
			} else if (type == ExpandCollapseAnimation.COLLAPSE) {
				expandCollapseListener.onCollapse(view, parentView, position);
			}
		}
	...
	public void enableFor(View parent, int position) {
		...
		enableFor(more, itemToolbar, parent, position);
		itemToolbar.requestLayout();
	}

	private void enableFor(final View button, final View target, final View parentView, final int position) {
				...
				if (a != null && a.hasStarted() && !a.hasEnded()) {
					...
					if (type == ExpandCollapseAnimation.EXPAND) {
						if (lastOpenPosition != -1 && lastOpenPosition != position) {
							if (lastOpen != null && lastParent != null) {
								animateView(lastOpen, ExpandCollapseAnimation.COLLAPSE);
								notifiyExpandCollapseListener(ExpandCollapseAnimation.COLLAPSE, lastOpen, lastParent,
										lastOpenPosition);
							}
					...
					notifiyExpandCollapseListener(type, target, parentView, position);
				}
			}
		});
	}
	...
	public boolean collapseLastOpen() {
		if (isAnyItemExpanded()) {
			...
			if (lastOpen != null) {
				animateView(lastOpen, ExpandCollapseAnimation.COLLAPSE);
				//				notifiyExpandCollapseListener(ExpandCollapseAnimation.COLLAPSE, lastOpen, lastParent, lastOpenPosition);
			}
			openItems.set(lastOpenPosition, false);
			lastOpenPosition = -1;
			return true;
